...
async def update_token(file: UploadFile = File(...)):
    """Receives uploaded tokens.db file from remote client."""
    try:
        token_path = Path(__file__).resolve().parent.parent.parent / "secrets" / "tokens.db"
        with open(token_path, "wb") as buffer:
            shutil.copyfileobj(file.file, buffer)
        return {"status": "Success", "path": str(token_path)}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
